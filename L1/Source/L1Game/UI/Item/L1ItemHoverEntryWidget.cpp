...
#include UE_INLINE_GENERATED_CPP_BY_NAME(L1ItemHoverEntryWidget)

#define LOCTEXT_NAMESPACE "ItemHoverWidget"

UL1ItemHoverEntryWidget::UL1ItemHoverEntryWidget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
    
}

void UL1ItemHoverEntryWidget::NativePreConstruct()
{
...
void UL1ItemHoverEntryWidget::RefreshUI(const UL1ItemInstance* ItemInstance)
{
	...
	FColor RarityColor = UL1UIData::Get().GetRarityColor(ItemRarity);
	...
	UTexture2D* RarityTexture = UL1UIData::Get().GetHoverRarityTexture(ItemRarity);
	...
	// Item Specifics
	if (const UL1ItemFragment_Equipable* EquipableFragment = ItemTemplate.FindFragmentByClass<UL1ItemFragment_Equipable>())
	{
		if (EquipableFragment->EquipmentType == EEquipmentType::Armor)
		{
			const UL1ItemFragment_Equipable_Armor* ArmorFragment = Cast<UL1ItemFragment_Equipable_Armor>(EquipableFragment);
		...
		{
			const UL1ItemFragment_Equipable_Weapon* WeaponFragment = Cast<UL1ItemFragment_Equipable_Weapon>(EquipableFragment);
		...
		{
			const UL1ItemFragment_Equipable_Utility* UtilityFragment = Cast<UL1ItemFragment_Equipable_Utility>(EquipableFragment);
		...
		const FL1GameplayTagStackContainer& StackContainer = ItemInstance->GetStatContainer();
		for (const FL1GameplayTagStack& Stack : StackContainer.GetStacks())
		{
			FString Left, Right;
		...
		else
		{
			for (int32 i = 0; i < (int32)ECharacterClassType::Count; i++)
			{
				...
				const FL1ClassInfoEntry& ClassInfoEntry = UL1ClassData::Get().GetClassInfoEntry(CharacterClassType);

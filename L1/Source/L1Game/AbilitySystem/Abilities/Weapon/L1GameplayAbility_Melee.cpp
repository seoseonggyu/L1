	...
	for (int32 i = 0; i < InTargetDataHandle.Data.Num(); i++)
	{
		...
		if (FHitResult* HitResult = const_cast<FHitResult*>(TargetData->GetHitResult()))
		{
			if (AActor* HitActor = HitResult->GetActor())
			{
				...
				// SSG: Team
				/*IL1TeamAgentInterface* TeamAgentInterface = Cast<IL1TeamAgentInterface>(TargetCharacter);
				...
				AL1EquipmentBase* HitWeaponActor = Cast<AL1EquipmentBase>(HitActor);
...
void UL1GameplayAbility_Melee::ProcessHitResult(FHitResult HitResult, float Damage, bool bBlockingHit, UAnimMontage* BackwardMontage, AL1EquipmentBase* WeaponActor)
{
	...
	EffectSpecHandle.Data->SetSetByCallerMagnitude(L1GameplayTags::SetByCaller_BaseDamage, 20.f);

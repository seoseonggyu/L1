...
void ULyraPawnExtensionComponent::BeginPlay()
{
	...
	ensure(TryToChangeInitState(L1GameplayTags::InitState_Spawned));
...
void ULyraPawnExtensionComponent::SetPawnData(const ULyraPawnData* InPawnData)
{
	...
	if (PawnData)
	{
		UE_LOG(LogL1, Error, TEXT("Trying to set PawnData [%s] on pawn [%s] that already has valid PawnData [%s]."), *GetNameSafe(InPawnData), *GetNameSafe(Pawn), *GetNameSafe(PawnData));
		return;
	}
...
void ULyraPawnExtensionComponent::InitializeAbilitySystem(ULyraAbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	...
	UE_LOG(LogL1, Verbose, TEXT("Setting up ASC [%s] on pawn [%s] owner [%s], existing [%s] "), *GetNameSafe(InASC), *GetNameSafe(Pawn), *GetNameSafe(InOwnerActor), *GetNameSafe(ExistingAvatar));

	if ((ExistingAvatar != nullptr) && (ExistingAvatar != Pawn))
	{
		UE_LOG(LogL1, Log, TEXT("Existing avatar (authority=%d)"), ExistingAvatar->HasAuthority() ? 1 : 0);
...
void ULyraPawnExtensionComponent::CheckDefaultInitialization()
{
	...
	static const TArray<FGameplayTag> StateChain = { L1GameplayTags::InitState_Spawned, L1GameplayTags::InitState_DataAvailable, L1GameplayTags::InitState_DataInitialized, L1GameplayTags::InitState_GameplayReady };
...
bool ULyraPawnExtensionComponent::CanChangeInitState(UGameFrameworkComponentManager* Manager, FGameplayTag CurrentState, FGameplayTag DesiredState) const
{
	...
	if (!CurrentState.IsValid() && DesiredState == L1GameplayTags::InitState_Spawned)
	{
	...
	if (CurrentState == L1GameplayTags::InitState_Spawned && DesiredState == L1GameplayTags::InitState_DataAvailable)
	{
	...
	else if (CurrentState == L1GameplayTags::InitState_DataAvailable && DesiredState == L1GameplayTags::InitState_DataInitialized)
	{
		// Transition to initialize if all features have their data available
		return Manager->HaveAllFeaturesReachedInitState(Pawn, L1GameplayTags::InitState_DataAvailable);
	}
	else if (CurrentState == L1GameplayTags::InitState_DataInitialized && DesiredState == L1GameplayTags::InitState_GameplayReady)
	{
		return true;
	}
...
{
	if (DesiredState == L1GameplayTags::InitState_DataInitialized)
	{
...
void ULyraPawnExtensionComponent::OnActorInitStateChanged(const FActorInitStateChangedParams& Params)
{
	...
	{
		if (Params.FeatureState == L1GameplayTags::InitState_DataAvailable)
		{
			CheckDefaultInitialization();
		}
	}

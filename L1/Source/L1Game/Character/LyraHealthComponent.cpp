...
void ULyraHealthComponent::InitializeWithAbilitySystem(ULyraAbilitySystemComponent* InASC)
{
	...
	HealthSet = AbilitySystemComponent->GetSet<UL1VitalSet>();
	if (!HealthSet)
	{
	...
	AbilitySystemComponent->SetNumericAttributeBase(UL1VitalSet::GetHealthAttribute(), HealthSet->GetMaxHealth());
...
void ULyraHealthComponent::DamageSelfDestruct(bool bFellOutOfWorld)
{
	if ((DeathState == ELyraDeathState::NotDead) && AbilitySystemComponent)
	{
		...
		Spec->SetSetByCallerMagnitude(L1GameplayTags::SetByCaller_BaseDamage, DamageAmount);
